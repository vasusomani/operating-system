...
void sjfScheduling(struct Process *processes, int n)
{
    ...
    while (i < n)
    {
        ...
        {
            processes[min_j].compl_time = curr_time + processes[min_j].burst_time;
            processes[min_j].is_comp = 1;
            i++;

            processes[min_j].tat_time = processes[min_j].compl_time - processes[min_j].arr_time;
            processes[min_j].wait_time = processes[min_j].tat_time - processes[min_j].burst_time;

            curr_time = processes[min_j].compl_time;
        }
    }
}
